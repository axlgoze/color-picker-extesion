...
export function saveColorToStorage(color) {
  chrome.storage.local.set({ hexFormat: color });
}

export function getColorFromStorage() {
  return new Promise((resolve) => {
    chrome.storage.local.get('hexFormat', (result) => {
      resolve(result.hexFormat);
    });
  });
}
...
export function generateFullPallete(hexFormat) {
  ...
  return [
    ...
    {
      hex: rgbToHex(s0.r, s0.g, s0.b),
      rgb: `rgb(${s0.r}, ${s0.g}, ${s0.b})`,
      label: 'similar',
    },
    {
      hex: rgbToHex(s1.r, s1.g, s1.b),
      rgb: `rgb(${s1.r}, ${s1.g}, ${s1.b})`,
      label: 'similar',
    },
    {
      hex: rgbToHex(s2.r, s2.g, s2.b),
      rgb: `rgb(${s2.r}, ${s2.g}, ${s2.b})`,
      label: 'similar',
    },
    {
      hex: rgbToHex(comp.r, comp.g, comp.b),
      rgb: `rgb(${comp.r}, ${comp.g}, ${comp.b})`,
      label: 'complementary',
    },
  ];
}
